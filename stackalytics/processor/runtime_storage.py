...
class MemcachedStorage(RuntimeStorage):
    ...
    def __init__(self, uri):
        ...
        if stripped:
            ...
            self.memcached = pylibmc.Client(storage_uri)
            self._build_index()
        else:
    ...
    def set_records(self, records_iterator):
        for record in records_iterator:
            if record['commit_id'] in self.commit_id_index:
                ...
                record['branches'][old_record['branches']] = True
    ...
    def get_update(self, pid):
        ...
        update_count = self.memcached.get('update:count')
    ...
    def active_pids(self, pids):
        stored_pids = self.memcached.get('pids') or []
        ...
        min_update = self.memcached.get('update:count')
        for pid in pids:
    ...
    def _commit_update(self, record_id):
        count = self.memcached.get('update:count') or 0
