...
class RecordProcessor(object):
    ...
    def update_user(self, record):
        ...
        if (CONF.query_launchpad and email and (not user_e) and
        ...
        if gerrit_id:
            ...
            if (CONF.query_launchpad and (not user_g) and
                    (not launchpad_id) and (not user_e.get('launchpad_id'))):
                # query LP
        ...
        if zanata_id:
            ...
            if (CONF.query_launchpad and (not user_z) and
                    (not launchpad_id) and (not user_e.get('launchpad_id'))):
                # query LP
        ...
        if ((user_e.get('seq') == user_l.get('seq') == user_g.get('seq') ==
            ...
            if user_e or user_l or user_g or user_z:
                ...
                # create new
                if (CONF.query_launchpad and not user_name):
