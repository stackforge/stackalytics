...
OPTS = [
    ...
    cfg.BoolOpt('read-default-data', default=False,
...
def process_repo(repo, runtime_storage, processor):
    ...
    vcs = VcsFactory.get_vcs(uri)
    vcs.fetch()

    os.chdir(cfg.CONF.sources_root + '/' + vcs.dir)
    ...
    for branch in repo['branches']:
        ...
        processed_commit_iterator = processor.process(commit_iterator, repo,
                                                      branch)
...
def update_repos(runtime_storage, persistent_storage):
    ...
    processor = commit_processor.CommitProcessorFactory.get_processor(
        commit_processor.COMMIT_PROCESSOR_DUMMY,
        persistent_storage)
