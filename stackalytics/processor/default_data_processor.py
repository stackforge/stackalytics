...
def _retrieve_project_list_from_gerrit(project_sources, gerrit):
        ...
        for repo in git_repos:
            ...
            if name not in exclude:
                r = {
                    ...
                    'uri': 'git://git.openstack.org/%s.git' % repo,
                    'releases': []
                }
                repos.append(r)
...
def _update_project_list(default_data):
    ...
    repos = _retrieve_project_list_from_gerrit(
        default_data['project_sources'], gerrit)
    if repos:
...
def process(runtime_storage_inst, default_data, sources_root, force_update,
    ...
    if 'project_sources' in default_data:
        _update_project_list(default_data, gerrit)
