...
Change-Id: Id26fdfd2af4862652d7270aec132d40662efeb96
    ...
    def test_log_commit_with_merge(self, mock_gli, mock_fetch):
        ...
        source_commit = Commit(
            ...
            stats=Stats(dict(insertions=12, deletions=23, files=2)))
        in_master_commit = Commit(
            hexsha=uuid.uuid4().hex,
            author=Author('Homer Simpson', 'homer@simpson.name'),
            authored_date=1234567800,
            parents=[],
            summary='Initial commit',
            message='Initial commit',
            stats=Stats(dict(insertions=0, deletions=0, files=0)))
        merge_parent_ids = [in_master_commit, source_commit]
        merge_commit = Commit(
            hexsha=commit_id,
        ...
        expected_commit = {
            'author_email': 'homer@simpson.name',
            'author_name': 'Homer Simpson', 'author_date': 1234567890,
            'branches': {'master'},
            'commit_id': commit_id,
            'date': 1234567899,
            'files_changed': 2, 'lines_added': 12, 'lines_deleted': 23,
